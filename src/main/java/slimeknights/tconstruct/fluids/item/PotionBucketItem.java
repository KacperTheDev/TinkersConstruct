...
public class PotionBucketItem extends PotionItem {
  ...
  public String getDescriptionId(ItemStack stack) {
    var potion = stack.getOrDefault(DataComponents.POTION_CONTENTS, PotionContents.EMPTY).potion().orElse(Potions.WATER);
    String bucketKey = potion.value().getName(Optional.of(potion), getDescriptionId() + ".effect.");
  ...
  public Component getName(ItemStack stack) {
    var potionHolder = stack.getOrDefault(DataComponents.POTION_CONTENTS, PotionContents.EMPTY).potion().orElse(Potions.WATER);
    Potion potion = potionHolder.value();
    String bucketKey = potion.getName(Optional.of(potionHolder), getDescriptionId() + ".effect.");
    if (Util.canTranslate(bucketKey)) {
      return Component.translatable(bucketKey);
    }
    // default to filling with the contents
    return Component.translatable(getDescriptionId() + ".contents", Component.translatable(potion.getName(Optional.of(potionHolder), "item.minecraft.potion.effect.")));
  }
  ...
  public void appendHoverText(ItemStack pStack, Item.TooltipContext context, List<Component> pTooltip, TooltipFlag pFlag) {
    pStack.getOrDefault(DataComponents.POTION_CONTENTS, PotionContents.EMPTY).addPotionTooltip(pTooltip::add, 2.5f, 20.0f);
  }
