  ...
  void registerCapabilities(RegisterCapabilitiesEvent event) {
    for (Item item : BuiltInRegistries.ITEM) {
      if (item instanceof PotionBucketItem bucket) {
        event.registerItem(Capabilities.FluidHandler.ITEM, (stack, context) -> bucket.getFluidHandler(stack), item);
      } else if (item instanceof MagmaBottleItem bottle) {
        event.registerItem(Capabilities.FluidHandler.ITEM, (stack, context) -> bottle.getFluidHandler(stack), item);
      } else if (item instanceof FluidContainerFoodItem food) {
        event.registerItem(Capabilities.FluidHandler.ITEM, (stack, context) -> food.getFluidHandler(stack), item);
      }
    }
