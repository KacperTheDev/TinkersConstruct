...
public class JEIPlugin implements IModPlugin {
  ...
  public void registerVanillaCategoryExtensions(IVanillaCategoryExtensionRegistration registry) {
    // JEI 19.39 changed crafting extensions from per-recipe factories to stateless extension instances.
    // The legacy material-aware crafting extensions are intentionally deferred; vanilla crafting display still works.
  }
    ...
    if (Config.CLIENT.showPotionFluidInJEI.get()) {
      manager.addIngredientsAtRuntime(NeoForgeTypes.FLUID_STACK,
                                      BuiltInRegistries.POTION.holders()
                                        ...
                                        .map(holder -> PotionFluidType.potionFluid(holder.key(), FluidType.BUCKET_VOLUME))
                                        .toList());
    }
