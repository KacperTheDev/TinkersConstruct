...
public class TConstruct {
  ...
  public TConstruct(IEventBus bus, ModContainer container) {
    ...
    TinkerNetwork.setup();
    bus.addListener(EventPriority.NORMAL, false, RegisterPayloadHandlersEvent.class, TinkerNetwork.getInstance()::registerPayload);
    TinkerTags.init();
    // init client logic
    if (FMLEnvironment.dist == Dist.CLIENT) { TinkerClient.onConstruct(); }

    // TODO 1.21 port: re-enable optional compat modules once their dependencies are ported.
  }
  ...
  static void gatherData(final GatherDataEvent event) {
    ...
    // other datagen
    generator.addProvider(server, new TConstructLootTableProvider(packOutput, event.getLookupProvider()));
