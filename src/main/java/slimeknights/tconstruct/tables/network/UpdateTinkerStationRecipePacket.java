...
public class UpdateTinkerStationRecipePacket implements IThreadsafePacket {
  ...
  private static class HandleClient {
    private static void handle(UpdateTinkerStationRecipePacket packet) {
      ...
      if (world != null) {
        RecipeHolder<ITinkerStationRecipe> recipe = world.getRecipeManager().byKey(packet.recipe)
          .flatMap(holder -> holder.value() instanceof ITinkerStationRecipe stationRecipe
                             ? Optional.of(new RecipeHolder<>(holder.id(), stationRecipe))
                             : Optional.empty())
          .orElse(null);
        ...
        if (!handled) {
          if (recipe != null) {
            BlockEntityHelper.get(TinkerStationBlockEntity.class, world, packet.pos).ifPresent(te -> te.updateRecipe(recipe));
          }
        }
      }
    }
