...
public class FluidEffectProjectile extends Projectile implements ProjectileWithKnockback, ProjectileWithPower {
  ...
  private IItemHandlerModifiable getCannonInventory() {
    ...
    if (this.cannon != null && level.isLoaded(this.cannon)) {
      var handler = level.getCapability(Capabilities.ItemHandler.BLOCK, this.cannon, null);
  ...
  protected void addAdditionalSaveData(CompoundTag nbt) {
    ...
    if (cannon != null) {
      nbt.put(KEY_CANNON, TagUtil.writeBlockPos(cannon));
    }
    FluidStack fluid = getFluid();
    if (!fluid.isEmpty()) {
      nbt.put(KEY_FLUID, fluid.save(level().registryAccess(), new CompoundTag()));
    }
  }
  ...
  protected void readAdditionalSaveData(CompoundTag nbt) {
    ...
    if (nbt.contains(KEY_CANNON)) {
      this.cannon = TagUtil.readBlockPos(nbt.get(KEY_CANNON));
    } else {
      this.cannon = null;
    }
